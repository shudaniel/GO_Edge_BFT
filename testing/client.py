...
def client_thread(id, ip, port, num_transactions, percent, zone, primaries, times, sem):
    ...
    for i in range(num_transactions):
        ...
        reply = data.decode()[:-1]
        print("Received reply", reply, msgtype)
...
for i in range(num_clients):
    ...
    new_thread = threading.Thread(target=client_thread, args=(i, args.address, args.port + i + 1, num_transactions, args.percent, args.zone, primaries, times, sem))
...
final_msg = str(times_sum) + "|-------|" + str(len(times)) + "|*"
